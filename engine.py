...
def engine(flag, key, trade_exposure):
    flag = flag
    key = key
    ...
    while True:
        ...
        if flag == 1:
            ...
            cancel_event = bs.limit_crypto_buy('BTC', trade_exposure, ladder=True)
            if cancel_event == 0:
                flag = -1
            if cancel_event == 1:
                flag = 1
        if flag == -1:
            flag = gd.Global_Dictionary[key](flag)
            cancel_event = bs.limit_crypto_sell('BTC', trade_exposure, ladder=True)
            if cancel_event == 0:
                flag = 1
            if cancel_event == -1:
                flag = -1
