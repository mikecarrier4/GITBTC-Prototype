...
def buy_crypto (trading_asset, trade_exposure, side):
...
def sell_crypto (trading_asset, trade_exposure, side):
    order = r.orders.order_sell_crypto_by_price(
    symbol = 'BTC',
    amountInDollars = float(trade_exposure),
    timeInForce='gtc',
    jsonify = True)
    print('Trade sent to Buy')
    #TODO check for status of Trade
    u.time_wait(15)
    print(order)
...
def limit_crypto_sell (trading_asset, trade_exposure, ladder=True):
    ...
    amount = amount_calc_sell(trade_exposure)
    print('amount', amount)
    ...
    while True:
        ...
        if (float(order_check['price'])) - (c.Average_Price_Crypto_Call().get_btc()) > 200:
            r.orders.cancel_crypto_order(order_id)
            print('Cancelling Now, the current order price was', 
            order_check['price'], 'and the current btc price was',
            c.Average_Price_Crypto_Call.get_btc())
            return 1
        else:
...
def amount_calc_sell (trade_exposure):
    position = r.crypto.get_crypto_positions()
    bit_quote = r.crypto.get_crypto_quote(symbol='BTC')
